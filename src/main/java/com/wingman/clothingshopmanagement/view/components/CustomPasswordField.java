...
public class CustomPasswordField extends JPasswordField{
    ...
    public CustomPasswordField() {
        eye = new ImageIcon(getClass().getResource("/images/show.png")).getImage();
        eye_hide = new ImageIcon(getClass().getResource("/images/hide.png")).getImage();
        addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                int x = getWidth() - 35;
                if (new Rectangle(x, 0 ,32,32).contains(e.getPoint())) {
                    hide = !hide;
                }
                if (hide) {
                    setEchoChar('•');
